<template>
  <div>
      ...
      <div class="container">
        <div class="checkout-addr">
          ...
          <div class="next-btn-wrap">
            <router-link
              :to="{path:'orderConfirm', query:{'addressId': selectedAddressId}}"
    ...
    <modal :mdShow="isMdShow" @closeModal="closeModal">
      <span slot="message">是否确认要删除此地址</span>
      <div slot="btnGroup">
...
export default {
  ...
  methods: {
    init() {
      this.axios.get("/apis/users/addressList").then((response) => {
    ...
    setDefault(addressId) {
      this.axios.post('/apis/users/setDefault', {
        addressId: addressId
      }).then((response) => {
        let res = response.data;
        if (res.status == '0') {
    ...
    delAddress() {
      this.axios.post("/apis/users/delAddress", {
